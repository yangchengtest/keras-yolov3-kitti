...
def detect_imgs(yolo,s_dir, d_dir):
    if not os.path.exists(s_dir):
        print("source dir is not exist!")
    if not os.path.exists(d_dir):
        os.makedirs(d_dir)
    inputimgs=glob(s_dir+"/*")
    for img in inputimgs:
        ...
        out_imgpath=d_dir+img[len(s_dir):]
        r_img.save(out_imgpath)

if __name__ == '__main__':
    arg=parser.parse_args()
    if (arg.s_dir) and (arg.d_dir):
        detect_imgs(YOLO(), arg.s_dir, arg.d_dir)
    else:
        detect_img(YOLO())
